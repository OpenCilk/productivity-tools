...
# Because cilktools spends a lot of time setting up custom compile flags,
...
macro(append_rtti_flag polarity list)
  if(${polarity})
    append_list_if(CILKTOOLS_HAS_FRTTI_FLAG -frtti ${list})
    append_list_if(CILKTOOLS_HAS_GR_FLAG /GR ${list})
  else()
    append_list_if(CILKTOOLS_HAS_FNO_RTTI_FLAG -fno-rtti ${list})
    append_list_if(CILKTOOLS_HAS_GR_FLAG /GR- ${list})
  endif()
endmacro()
...
macro(test_target_arch arch def)
  ...
  if(${CAN_TARGET_${arch}})
    list(APPEND CILKTOOLS_SUPPORTED_ARCH ${arch})
  elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" STREQUAL "${arch}" AND
         CILKTOOLS_HAS_EXPLICIT_DEFAULT_TARGET_TRIPLE)
...
function(get_cilktools_root_source_dir ROOT_DIR_VAR)
  # Compute the path to the root of the Cilktools source tree
  # regardless of how the project was configured.
  #
  # This function is useful because using `${CMAKE_SOURCE_DIR}`
  # is error prone due to the numerous ways Cilktools can be
  # configured.
  #
  ...
  # cilktools supports different source root paths.
  # Handle each case here.
  set(PATH_TO_CILKTOOLS_SOURCE_ROOT "")
  if(DEFINED Cilktools_SOURCE_DIR)
    # Cilktools standalone build.
    # `opencilk-project/cilktools`
    set(PATH_TO_CILKTOOLS_SOURCE_ROOT "${Cilktools_SOURCE_DIR}")
  elseif (EXISTS "${CMAKE_SOURCE_DIR}/../cilktools")
    # In tree build with LLVM as the root project.
    # See `llvm-project/projects/`.
    # Assumes monorepo layout.
    set(PATH_TO_CILKTOOLS_SOURCE_ROOT "${CMAKE_SOURCE_DIR}/../cilktools")
  else()
    message(FATAL_ERROR "Unhandled Cilktools source root configuration.")
  endif()

  get_filename_component(ROOT_DIR "${PATH_TO_CILKTOOLS_SOURCE_ROOT}" ABSOLUTE)
...
macro(load_llvm_config)
  ...
  # CMake cache value.
  get_cilktools_root_source_dir(CILKTOOLS_ROOT_SRC_PATH)
  get_filename_component(LLVM_MAIN_SRC_DIR_DEFAULT "${CILKTOOLS_ROOT_SRC_PATH}/../llvm" ABSOLUTE)
  ...
  if (NOT LLVM_FOUND)
    ...
    if (XRAY_INDEX EQUAL -1)
      ...
      set(CILKTOOLS_HAS_LLVMXRAY FALSE)
    endif()
...
macro(construct_cilktools_default_triple)
  if(CILKTOOLS_DEFAULT_TARGET_ONLY)
    if(DEFINED CILKTOOLS_DEFAULT_TARGET_TRIPLE)
      message(FATAL_ERROR "CILKTOOLS_DEFAULT_TARGET_TRIPLE isn't supported when building for default target only")
    endif()
...
function(get_cilktools_target arch variable)
  string(FIND ${CILKTOOLS_DEFAULT_TARGET_TRIPLE} "-" dash_index)
  string(SUBSTRING ${CILKTOOLS_DEFAULT_TARGET_TRIPLE} ${dash_index} -1 triple_suffix)
  if(CILKTOOLS_DEFAULT_TARGET_ONLY)
...
function(get_cilktools_install_dir arch install_dir)
  if(LLVM_ENABLE_PER_TARGET_RUNTIME_DIR AND NOT APPLE)
    get_cilktools_target(${arch} target)
    set(${install_dir} ${CILKTOOLS_INSTALL_LIBRARY_DIR}/${target} PARENT_SCOPE)
  else()
    set(${install_dir} ${CILKTOOLS_INSTALL_LIBRARY_DIR} PARENT_SCOPE)
  endif()
endfunction()

function(get_cilktools_output_dir arch output_dir)
  if(LLVM_ENABLE_PER_TARGET_RUNTIME_DIR AND NOT APPLE)
    get_cilktools_target(${arch} target)
    set(${output_dir} ${CILKTOOLS_OUTPUT_LIBRARY_DIR}/${target} PARENT_SCOPE)
  else()
    set(${output_dir} ${CILKTOOLS_OUTPUT_LIBRARY_DIR} PARENT_SCOPE)
  endif()
endfunction()

# cilktools_process_sources(
#   <OUTPUT_VAR>
#   <SOURCE_FILE> ...
...
# because we need to support standalone builds of cilktools.
function(cilktools_process_sources OUTPUT_VAR)
...
function(add_cilktools_install_targets name)
  ...
  if(ARG_PARENT_TARGET AND NOT TARGET install-${ARG_PARENT_TARGET})
    ...
    set_target_properties(install-${ARG_PARENT_TARGET}-stripped PROPERTIES
                          FOLDER "Cilktools Misc")
    add_dependencies(install-cilktools install-${ARG_PARENT_TARGET})
    add_dependencies(install-cilktools-stripped install-${ARG_PARENT_TARGET}-stripped)
  endif()
