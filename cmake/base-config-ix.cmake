...
add_custom_target(install-cilktools)
add_custom_target(install-cilktools-stripped)
set_property(
  TARGET
    cilktools
    install-cilktools
    install-cilktools-stripped
  PROPERTY
...
if (LLVM_TREE_AVAILABLE)
  ...
  set(CILKTOOLS_TEST_COMPILER
    ${LLVM_RUNTIME_OUTPUT_INTDIR}/clang${_host_executable_suffix})
  set(CILKTOOLS_TEST_CXX_COMPILER
    ${LLVM_RUNTIME_OUTPUT_INTDIR}/clang++${_host_executable_suffix})
else()
    # Take output dir and install path from the user.
  set(CILKTOOLS_OUTPUT_DIR ${CMAKE_CURRENT_BINARY_DIR} CACHE PATH
    "Path where built cilktools libraries should be stored.")
  set(CILKTOOLS_EXEC_OUTPUT_DIR ${CMAKE_CURRENT_BINARY_DIR}/bin CACHE PATH
    "Path where built cilktools executables should be stored.")
  set(CILKTOOLS_INSTALL_PATH ${CMAKE_INSTALL_PREFIX} CACHE PATH
    "Path where built cilktools libraries should be installed.")
  option(CILKTOOLS_INCLUDE_TESTS "Generate and build cilktools unit tests." OFF)
  option(CILKTOOLS_ENABLE_WERROR "Fail and stop if warning is triggered" OFF)
  # Use a host compiler to compile/link tests.
  set(CILKTOOLS_TEST_COMPILER ${CMAKE_C_COMPILER} CACHE PATH "Compiler to use for testing")
  set(CILKTOOLS_TEST_CXX_COMPILER ${CMAKE_CXX_COMPILER} CACHE PATH "C++ Compiler to use for testing")
endif()

if("${CILKTOOLS_TEST_COMPILER}" MATCHES "clang[+]*$")
  set(CILKTOOLS_TEST_COMPILER_ID Clang)
elseif("${CILKTOOLS_TEST_COMPILER}" MATCHES "clang.*.exe$")
  set(CILKTOOLS_TEST_COMPILER_ID Clang)
else()
  set(CILKTOOLS_TEST_COMPILER_ID GNU)
endif()

if(NOT DEFINED CILKTOOLS_OS_DIR)
...
if(APPLE)
  ...
  option(CILKTOOLS_DEFAULT_TARGET_ONLY "Build builtins only for the default target" Off)
endif()
...
macro(test_targets)
  ...
  if (MSVC)
    ...
    if (MSVC_COMPAT_VERSION MATCHES "^[0-9].+$")
      ...
      elseif (CILKTOOLS_TEST_COMPILER_ID MATCHES "Clang")
        # Add this flag to test compiles to suppress clang's auto-detection
        # logic.
        append("${MSVC_VERSION_FLAG}" CILKTOOLS_TEST_COMPILER_CFLAGS)
      endif()
    endif()
  endif()

  # Generate the CILKTOOLS_SUPPORTED_ARCH list.
  if(ANDROID)
    # Examine compiler output to determine target architecture.
    detect_target_arch()
    set(CILKTOOLS_OS_SUFFIX "-android")
  elseif(NOT APPLE) # Supported archs for Apple platforms are generated later
    if(CILKTOOLS_DEFAULT_TARGET_ONLY)
      add_default_target_arch(${CILKTOOLS_DEFAULT_TARGET_ARCH})
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "i[2-6]86|x86|amd64")
      if(NOT MSVC)
    ...
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "powerpc")
    ...
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "s390x")
      test_target_arch(s390x "" "")
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "sparc")
      test_target_arch(sparc "" "-m32")
      test_target_arch(sparcv9 "" "-m64")
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "mipsel|mips64el")
    ...
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "mips")
      test_target_arch(mips "" "-mips32r2" "-mabi=32" "-D_LARGEFILE_SOURCE" "-D_FILE_OFFSET_BITS=64")
      test_target_arch(mips64 "" "-mips64r2" "-mabi=64")
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "arm")
      if(WIN32)
    ...
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "aarch32")
      test_target_arch(aarch32 "" "-march=armv8-a")
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "aarch64")
      test_target_arch(aarch64 "" "-march=armv8-a")
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "riscv32")
      test_target_arch(riscv32 "" "")
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "riscv64")
      test_target_arch(riscv64 "" "")
    elseif("${CILKTOOLS_DEFAULT_TARGET_ARCH}" MATCHES "wasm32")
