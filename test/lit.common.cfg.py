...
# This file contains common rules for various cilktools testsuites.
...
if config.emulator:
  ...
  compile_wrapper = os.path.join(config.cilktools_src_root, "test", "sanitizer_common", "android_commands", "android_compile.py") + " "
  config.compile_wrapper = compile_wrapper
...
cilktools_debug = getattr(config, 'cilktools_debug', False)
if not cilktools_debug:
  config.available_features.add('cilktools-optimized')

libdispatch = getattr(config, 'cilktools_intercept_libdispatch', False)
if libdispatch:
